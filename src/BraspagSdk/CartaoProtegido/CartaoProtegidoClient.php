...
class CartaoProtegidoClient
{
    ...
    function saveCreditCard(SaveCreditCardRequest $request, MerchantCredentials $credentials = null)
    {
        if (empty($request) || !isset($request))
            throw new InvalidArgumentException("Request is null");
        ...
        try
        {
            ...
            $body = "<soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">";
            $body .= "<soap:Body>";
            $body .= "<SaveCreditCard xmlns=\"http://www.cartaoprotegido.com.br/WebService/\">";
            $body .= "<saveCreditCardRequestWS>";
            ...
            $body .= "<MerchantKey>{$currentCredentials->MerchantKey}</MerchantKey>";
            $body .= "<CustomerIdentification>{$request->CustomerIdentification}</CustomerIdentification>";
            $body .= "<CustomerName>{$request->CustomerName}</CustomerName>";
            $body .= "<CardHolder>{$request->CardHolder}</CardHolder>";
            $body .= "<CardNumber>{$request->CardNumber}</CardNumber>";
            $body .= "<CardExpiration>{$request->CardExpiration}</CardExpiration>";
            $body .= "<JustClickAlias>{$request->JustClickAlias}</JustClickAlias>";
            $body .= "<DataCollection />";
            $body .= "</saveCreditCardRequestWS>";
            $body .= "</SaveCreditCard>";
        ...
        if (!empty($httpResponse) || isset($httpResponse))
        {
            $response = SaveCreditCardResponse::fromXml($httpResponse);
            $response->HttpStatus = isset($statusCode) ? $statusCode : 0;
            return $response;
        }
        else
